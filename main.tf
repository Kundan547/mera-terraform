    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:s3:::datapemfile/*"
      }
    ]
  })
}
...
data "aws_s3_bucket" "pem_file_bucket" {
  bucket = "datapemfile"
}

resource "aws_s3_bucket_policy" "pem_bucket_policy" {
  bucket = data.aws_s3_bucket.pem_file_bucket.id
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
        ...
        Action   = ["s3:GetObject", "s3:PutObject"],
        Resource = "${data.aws_s3_bucket.pem_file_bucket.arn}/*"
      }
    ]
  })
}
